...
This is distinct from the **Adventure Engine** (`server/adventure.js`), which is the authoritative synced state machine for whichever adventure is currently loaded in a room (its acts/scenes/timers/encounters, plus climax-pacing state — `climaxPadScenes`/`climaxScenesSinceTrigger`/`climaxForced` — and a module's declared `persistence` Legacy Tracker schema, if any). A module is how you *get* an adventure onto a room's Adventure Engine (or into a player's local Adventure Manager) in the first place. (The Deck of Consequences, `server/deck.js`, is separate again — it now shuffles via a per-room seedable PRNG, `server/rng.js`, rather than a module concern.)
