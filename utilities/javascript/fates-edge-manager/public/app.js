...
document.addEventListener('click',async event=>{
  ...
  try{
    if(action==='create-room')modal('<h2>Create a room</h2><p>You will be its owner and GM. Your game role can be separate from administration.</p><form data-form="room"><label>Room name<input name="name" required maxlength="100" placeholder="Your campaign name"></label><button>Create room</button></form>');
      ...
      modal(`<h2>Connect an integration</h2><p>This key opens ${esc(room.name)} with your ${esc(room.role)} permissions.</p><form data-form="key" data-id="${id}"><label>Integration name<input name="label" required maxlength="100" placeholder="GM bot"></label><label>Expires in days<input type="number" name="days" min="1" max="365" value="90" required></label><p>Allowed actions</p>${room.scopes.map(s=>`<label class="check"><input type="checkbox" name="scopes" value="${s}" ${['room:read','room:connect'].includes(s)?'checked':''}>${s}</label>`).join('')}<button>Create key</button></form>`);
    ...
    else if(action==='connect'){const result=await api(`/rooms/${id}/connect`);reveal('Room connection',JSON.stringify(result,null,2),'This connection credential expires in ten minutes. Keep it private.');}
...
document.addEventListener('change',async event=>{
  const el=event.target;if(!el.dataset.member)return;
