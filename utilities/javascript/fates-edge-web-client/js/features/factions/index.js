 ...
 * - Discovery tries ./data/factions/manifest.json first (see
 *   js/tools/generate-manifests.js), falling back to testing a hardcoded
 *   list of known slugs (KNOWN_FACTION_SLUGS below) if no manifest is found.
 *
...
async function discoverFactions() {
    ...
    // Prefer a manifest.json (generated by js/tools/generate-manifests.js) if
    // present, so newly-added faction files show up without editing this
    // hardcoded array. Falls back to testing each known slug via HEAD.
    let slugs = KNOWN_FACTION_SLUGS;
    try {
        const res = await fetch(`${FACTION_DATA_PATH}manifest.json`);
        if (res.ok) {
    ...
    } catch (_) { /* fall through to KNOWN_FACTION_SLUGS */ }
