        ...
        listContainer.querySelectorAll('.bestiary-import-item').forEach(item => {
            item.addEventListener('click', () => {
                ...
                if (entry) {
                    ...
                    div.style.cssText = 'display:flex;gap:0.3rem;margin:0.2rem 0;align-items:center;';
                    div.innerHTML = `
                        <input type="text" class="adv-name" placeholder="Name" value="${escHtml(entry.name)}" style="flex:2;" />
                        <input type="text" class="adv-body" placeholder="Description / stats" value="${escHtml(getCreatureDescription(entry))}" style="flex:3;" />
                        <button class="btn btn-xs btn-danger adv-remove" data-index="${idx}">✕</button>
                    `;
                    list.appendChild(div);
