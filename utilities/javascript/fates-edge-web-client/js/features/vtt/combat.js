 ...
 * ✅ NEW: Position tracking (Dominant/Controlled/Desperate) per combatant,
 *    with a bridge maneuvers in the VTT's Combat Actions panel can use to
 *    automatically worsen/improve a character's tracked Position.
 * ✅ NEW: In-tracker Adversary Moves — click a generic move (Flurry,
 *    Grapple, Sunder, Enrage) or an adversary's own custom Tactics
 *    (set in the Encounter Editor) to log it straight to the combat log.
 * ✅ NEW: logExternalAction() lets other features (e.g. a player's roll
 *    from the VTT's Quick Roller) append a line to the GM's combat log
 *    without needing GM-only access to the Tracker itself.
 */
...
// ============================================================
// 👇 NEW: POSITION TRACKING (Dominant / Controlled / Desperate)
// ============================================================

// { combatantId: 'dominant' | 'controlled' | 'desperate' }
// Per-combatant (not per-pair like Range) — Position reflects how each
// individual is faring in the fight overall, matching how the core
// rules use it to set roll DV/edge.
let positionMap = {};

const POSITION_STATES = [
    { key: 'dominant', label: 'Dominant', short: 'Dom', color: 'var(--green)', desc: 'In control of the exchange — edge in your favor.' },
    { key: 'controlled', label: 'Controlled', short: 'Ctrl', color: 'var(--gold)', desc: 'Even footing.' },
    { key: 'desperate', label: 'Desperate', short: 'Desp', color: 'var(--red)', desc: 'On the back foot — edge against you.' }
];
const POSITION_ORDER = ['dominant', 'controlled', 'desperate'];
const DEFAULT_POSITION = 'controlled';

function getPositionKey(id) {
    return positionMap[id] || DEFAULT_POSITION;
}

function getPositionInfo(id) {
    const key = getPositionKey(id);
    return POSITION_STATES.find(p => p.key === key) || POSITION_STATES[1];
}

function setPositionKey(id, posKey) {
    if (!POSITION_ORDER.includes(posKey)) return;
    positionMap[id] = posKey;
}

function cyclePosition(id) {
    const idx = POSITION_ORDER.indexOf(getPositionKey(id));
    const next = POSITION_ORDER[(idx + 1) % POSITION_ORDER.length];
    setPositionKey(id, next);
}

function worsenPositionInternal(id) {
    const idx = POSITION_ORDER.indexOf(getPositionKey(id));
    const next = POSITION_ORDER[Math.min(idx + 1, POSITION_ORDER.length - 1)];
    setPositionKey(id, next);
    return next;
}

function improvePositionInternal(id) {
    const idx = POSITION_ORDER.indexOf(getPositionKey(id));
    const next = POSITION_ORDER[Math.max(idx - 1, 0)];
    setPositionKey(id, next);
    return next;
}

// ============================================================
// 👇 NEW: ADVERSARY MOVES (generic reference, SB-costed)
// Mirrors encounters/index.js's ADVERSARY_MOVES quick-reference list,
// but kept as a local copy here so the Tracker can turn them into real
// clickable, loggable actions without importing from index.js (which
// already imports from this file — avoids a circular dependency).
// ============================================================
...
let expandedMovesId = null; // which combatant's Moves panel is expanded, if any
...
function clearRangeForCombatant(id) {
    ...
    delete positionMap[id]; // 👈 NEW: drop stale Position tracking too
}
...
// ============================================================
// 👇 NEW: ADVERSARY MOVES PANEL
// ============================================================

// Builds the expandable "🎯 Moves" panel for a single adversary combatant:
// the four generic SB-costed moves, plus any custom Tactics the GM set
// for this specific adversary in the Encounter Editor (combatant.tactics).
function buildMovesHtml(combatant) {
    const customMoves = (combatant.tactics || []).map(name => ({ cost: null, name, effect: 'Custom tactic — GM adjudicates effect.' }));
    const allMoves = [...ADVERSARY_MOVES, ...customMoves];

    const rows = allMoves.map((m, i) => `
        <button class="move-btn" data-combatant="${combatant.id}" data-move-index="${i}"
            title="${escHtml(m.effect)}"
            style="
                display:flex; flex-direction:column; align-items:flex-start; gap:0.1rem;
                padding:0.35rem 0.6rem; background:var(--bg2); border:1px solid var(--border);
                border-radius:8px; cursor:pointer; text-align:left; min-width:120px;
            ">
            <span style="font-weight:600;font-size:0.8rem;color:var(--gold);">
                ${m.cost !== null ? `${m.cost} SB · ` : '🛠️ '}${escHtml(m.name)}
            </span>
            <span style="font-size:0.7rem;color:var(--text3);">${escHtml(m.effect)}</span>
        </button>
    `).join('');

    return `
        <div class="moves-panel" style="
            display:flex; flex-wrap:wrap; gap:0.4rem; margin:0.4rem 0 0.6rem 2.5rem;
            padding:0.5rem; background:var(--bg3); border-radius:10px; border:1px solid var(--border);
        ">
            ${rows}
        </div>
    `;
...
export function openTracker(encounterId) {
    ...
    if (encounter?.adversaries) {
        combatants = (encounter.adversaries || []).map(a => ({
            ...
            type: 'adversary',
            weaponClass: a.weaponClass || 'none', // 👈 NEW: from Encounter Editor's adversary combat profile
            tactics: Array.isArray(a.tactics) ? a.tactics : [], // 👈 NEW: custom per-adversary moves
    ...
    rangeMap = {};
    rangeGridOpen = false;
    positionMap = {}; // 👈 NEW: fresh Position tracking (everyone starts Controlled)
    expandedMovesId = null;
...
function renderTracker() {
    ...
    const combatantsHtml = combatants.map((c, i) => {
        ...
        // 👇 NEW: Position chip — always shown, per-combatant, click to cycle
        // Dominant → Controlled → Desperate → Dominant.
        const posInfo = getPositionInfo(c.id);
        const positionChip = `<span class="position-chip" data-id="${c.id}"
            title="Position: ${posInfo.label} — ${posInfo.desc} (click to cycle)"
            style="
                font-size:0.65rem; font-weight:700; color:white; background:${posInfo.color};
                padding:0.05rem 0.4rem; border-radius:10px; cursor:pointer; flex-shrink:0;
                letter-spacing:0.02em;
            ">🧭 ${posInfo.short}</span>`;
        ...
        // 👇 NEW: Moves toggle, adversaries only
        const movesToggle = c.type === 'adversary'
            ? `<button class="btn btn-xs btn-ghost combat-moves-toggle-btn" data-index="${i}"
                    style="padding: 0.25rem 0.4rem; font-size: 0.8rem; border-radius: 6px;"
                    title="Adversary moves">🎯</button>`
            : '';
        ...
        // 👇 NEW: expanded Moves panel directly under this row, if toggled open
        const movesPanel = (c.type === 'adversary' && expandedMovesId === c.id) ? buildMovesHtml(c) : '';

        return row + movesPanel;
    }).join('');
                ...
                ">
                    <h3 style="margin: 0; color: var(--gold); display: flex; align-items: center; gap: 0.5rem;">
                        ...
                        <span style="font-size:0.7rem;color:var(--text3);font-weight:400;">
                            (click to focus · 🧭 Position, 📏 Range: click chip to cycle · 🎯 adversary moves)
                        </span>
                    </h3>
    ...
    // 👇 NEW: Position chips — click to cycle Dominant → Controlled → Desperate
    modal.querySelectorAll('.position-chip').forEach(chip => {
        chip.addEventListener('click', (e) => {
            e.stopPropagation();
            cyclePosition(chip.dataset.id);
            renderTracker();
        });
    });
    // 👇 NEW: Adversary Moves toggle + move buttons
    modal.querySelectorAll('.combat-moves-toggle-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.stopPropagation();
            const idx = parseInt(btn.dataset.index);
            const c = combatants[idx];
            if (!c) return;
            expandedMovesId = expandedMovesId === c.id ? null : c.id;
            renderTracker();
        });
    });
    modal.querySelectorAll('.move-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.stopPropagation();
            const combatantId = btn.dataset.combatant;
            const moveIndex = parseInt(btn.dataset.moveIndex, 10);
            const c = combatants.find(c => c.id === combatantId);
            if (!c) return;
            const allMoves = [...ADVERSARY_MOVES, ...(c.tactics || []).map(name => ({ cost: null, name, effect: 'Custom tactic — GM adjudicates effect.' }))];
            const move = allMoves[moveIndex];
            if (!move) return;
            addLog('move', `${c.name} uses ${move.name}${move.cost !== null ? ` (${move.cost} SB)` : ''} — ${move.effect}`);
            renderTracker();
            showToast(`🎯 ${c.name} uses ${move.name}`, 'info');
        });
    });
                ...
                if (entry) {
                    ...
                    if (!exists) {
                        encounter.adversaries.push({
                            ...
                            tier: entry.tier || 2,
                            stats: entry.stats || {},
                            weaponClass: entry.weaponClass || 'none', // 👈 NEW
                            tactics: Array.isArray(entry.tactics) ? entry.tactics : [] // 👈 NEW
                        });
                        saveState();
...
function removeCombatant(idx) {
    if (idx >= 0 && idx < combatants.length) {
        if (confirm(`Remove ${combatants[idx].name}?`)) {
            ...
            clearRangeForCombatant(removedId); // 👈 NEW: drop stale range + position entries
            if (expandedMovesId === removedId) expandedMovesId = null;
...
// and the VTT's Combat Actions panel syncing maneuvers/rolls)
// ============================================================
...
// position). Lets other features (e.g. the Whiteboard or Combat Actions)
// pull in who's actually in the fight right now — including ad-hoc
// players added via "+ Player", which aren't persisted anywhere else —
// without touching the Tracker's own bookkeeping.
export function getLiveCombatants() {
    return combatants.map(c => ({
        id: c.id, name: c.name, type: c.type, status: c.status,
        harm: c.harm, maxHarm: c.maxHarm, position: getPositionKey(c.id)
    }));
}
...
// 👇 NEW: Position bridge — lets the VTT's Combat Actions panel read/set/
// worsen/improve a character's tracked Position when they use a maneuver
// (e.g. Sidestep Strike explicitly worsens Position by one step).
// All match by name (case-insensitive); all no-op quietly if the Tracker
// isn't open or the name isn't in the current fight.

export function getTrackerPositionByName(name) {
    if (!modal) return null;
    const c = combatants.find(c => (c.name || '').toLowerCase() === (name || '').toLowerCase());
    if (!c) return null;
    return getPositionKey(c.id);
}

export function setTrackerPositionByName(name, posKey) {
    if (!modal) return false;
    const c = combatants.find(c => (c.name || '').toLowerCase() === (name || '').toLowerCase());
    if (!c) return false;
    setPositionKey(c.id, posKey);
    renderTracker();
    return true;
}

export function worsenTrackerPositionByName(name) {
    if (!modal) return null;
    const c = combatants.find(c => (c.name || '').toLowerCase() === (name || '').toLowerCase());
    if (!c) return null;
    const newPos = worsenPositionInternal(c.id);
    renderTracker();
    return newPos;
}

export function improveTrackerPositionByName(name) {
    if (!modal) return null;
    const c = combatants.find(c => (c.name || '').toLowerCase() === (name || '').toLowerCase());
    if (!c) return null;
    const newPos = improvePositionInternal(c.id);
    renderTracker();
    return newPos;
}

// 👇 NEW: External action log bridge — lets other features (e.g. a
// player rolling from the VTT's Quick Roller) append a line to the GM's
// combat log, so the GM has one unified record instead of having to
// cross-reference the public chat separately. No-ops quietly if the
// Tracker isn't open — there's no log to append to yet.
export function logExternalAction(sourceName, message, type = 'action') {
    if (!modal) return false;
    addLog(type, `${sourceName ? sourceName + ': ' : ''}${message}`);
    renderTracker();
    return true;
}
